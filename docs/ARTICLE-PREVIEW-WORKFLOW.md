...
- 初稿先由 Gemini 2.5 Flash 使用免費 Google Search 整理論文，再由 Gemini 3.8 Flash 寫作；3.8 服務錯誤時改用 3.5 Flash-Lite，若仍回傳短文、缺章節或錯誤引用，最後由 2.5 Flash 依錯誤清單重寫修復。搜尋不足、額度受限或暫時失敗時仍繼續寫初稿，論文有幾篇列幾篇，零篇時明確標示且不得杜撰。系統仍會拒絕缺少必要版型、混入非論文來源或引用編號錯誤的最終結果。自動搜尋不等於人工審稿，正式使用前仍須逐篇打開論文核對。Google 免費服務可能用輸入改善產品，不輸入病患個資。
