...
async function collectGroundedResearch(topic: ContentTopic, key: string, deadline: number): Promise<string> {
  ...
  const continueWithoutResearch = (reason: string) => `【論文搜尋狀態】\n${reason}\n本次仍須繼續完成免費初稿，但只能撰寫保守的一般衛教內容。referencesHtml 只列下方既有線索中明確可核對的學術論文；若沒有，就保留參考文獻標題並寫明本次未取得可核對論文，絕對不可自行補造作者、DOI、PMID、網址或數據。\n\n【既有來源線索】\n${sourceSignals || '無'}`;
  const timeout = Math.max(10000, Math.min(90000, deadline - Date.now()));
  let response: Response;
  try {
    response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${FREE_DRAFT_RESEARCH_MODEL}:generateContent`, {
...
4. 不確定就明確寫不確定，絕對不能補造缺漏資料。即使不足 6 篇也要回傳已找到的內容，這份資料會交給另一個模型繼續寫初稿。` }] }],
        tools: [{ google_search: {} }],
        generationConfig: { temperature: 0.1, maxOutputTokens: 9000 },
      }),
    });
  } catch {
    return continueWithoutResearch('免費論文搜尋連線中斷或超過等待時間。');
  }
  if (!response.ok) {
    return continueWithoutResearch(response.status === 429 ? '免費論文搜尋額度已用完或請求過於頻繁。' : `免費論文搜尋暫時無法使用（HTTP ${response.status}）。`);
  }
  ...
  const webSources = (candidate?.groundingMetadata?.groundingChunks || candidate?.grounding_metadata?.grounding_chunks || [])
    ...
    .filter(Boolean).join('\n');
  if (!text || text.replace(/\s/g, '').length < 500) return continueWithoutResearch(`免費論文搜尋只回傳少量資料：${text || '沒有文字結果'}。`);
...
export function validateDraftArticle(article: ExternalArticleCode): void {
  ...
  if (!/總結摘要與核心觀點/.test(summary) || summaryCitations < Math.min(3, referenceItems.length)) missing.push('摘要卡或其中可用論文的引用');
  ...
  if (!/參考文獻|References/i.test(article.referencesHtml)) missing.push('參考文獻區塊');
  if (scholarly.length !== referenceItems.length) missing.push(`只能列附 DOI、PubMed、PMC 或期刊原始頁面的學術論文（目前 ${referenceItems.length} 筆，符合 ${scholarly.length} 筆）`);
  if (citations.length < Math.min(3, referenceItems.length) || citations.some((number) => number < 1 || number > referenceItems.length)) missing.push('與現有參考文獻編號一致的內文引用');
