...
const TOPIC_PROFILES: TopicProfile[] = [
  ...
  { key: 'low-back-pain', category: '脊椎下背', probe: '下背痛 怎麼辦', terms: ['下背痛', '腰痛', '閃到腰', 'low back pain'], excludeTerms: ['結石', '腎臟', '泌尿', '輸尿管', '胰臟', '腹腔'], titles: ['突然閃到腰怎麼辦？前 48 小時處理與危險警訊', '下背痛要休息還是運動？何時應該照 X 光或 MRI', '腰痛一直不好是椎間盤嗎？常見原因與就醫科別'], summary: '提供急慢性下背痛的安全活動原則、危險警訊與檢查時機，避免過度臥床或自行診斷。', rationale: '腰痛是極高頻症狀搜尋，也最需要可靠的分流與安全衛教。', keywords: ['閃到腰怎麼辦', '下背痛多久會好', '腰痛何時照MRI'], patientRelevance: 99, clinicalFit: 98 },
  { key: 'sciatica', category: '脊椎下背', probe: '坐骨神經痛 腳麻', terms: ['坐骨神經痛', '腳麻', 'sciatica', 'radiculopathy'], titles: ['屁股痛一路麻到腳，是坐骨神經痛嗎？', '坐骨神經痛可以拉筋嗎？哪些情況不適合自己做', '腳麻是腰椎壓迫嗎？需要立刻就醫的神經警訊'], summary: '區分神經根症狀與一般肌肉疼痛，整理居家活動、檢查與緊急就醫判斷。', rationale: '「腳麻、電到、一路痛」是病患常用搜尋語句，具高度門診轉換可能。', keywords: ['坐骨神經痛腳麻', '坐骨神經痛拉筋', '腳麻看哪一科'], patientRelevance: 98, clinicalFit: 97 },
...
async function fetchLiteratureSignals(): Promise<LiteratureSignal[]> {
  const term = '((musculoskeletal[Title/Abstract]) OR orthopedic[Title/Abstract] OR rehabilitation[Title/Abstract] OR sports medicine[Title/Abstract] OR physical therapy[Title/Abstract]) AND ("last 30 days"[PDat])';
  const searchUrl = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax=30&sort=pub+date&tool=dryichen_content_studio&term=${encodeURIComponent(term)}`;
...
function matchesProfile(text: string, profile: TopicProfile): boolean {
  ...
  return profile.terms.some((term) => normalized.includes(term.toLocaleLowerCase('zh-TW')));
}
...
export async function discoverDailyTopics(input: {
  ...
  const externalSignalCount = news.length + literature.length
  ...
  const candidates = TOPIC_PROFILES.map((profile) => {
    const newsMatches = news.filter((item) => matchesProfile(item.title, profile)).slice(0, 3);
    ...
    const evidenceTypes = Number(newsMatches.length > 0) + Number(literatureMatches.length > 0) + Number(suggestions.length > 0);
    ...
    for (const item of newsMatches.slice(0, 2)) {
      sources.push({ label: `近 7 日新聞：${item.title}`, sourceType: 'news', note: `Google News 近期報導訊號${item.publishedAt ? `，發布於 ${item.publishedAt.slice(0, 10)}` : ''}。`, url: item.url });
    }
    for (const item of literatureMatches.slice(0, 1)) {
      sources.push({ label: `近 30 日 PubMed：${item.title}`, sourceType: 'medical-literature', note: '近期收錄的醫學研究；撰稿時仍須核對研究設計與結論。', url: item.url });
    }
    if (!sources.length) {
    ...
    return {
      topic: {
        ...
        rationale: `${profile.rationale} 本次依 ${evidenceTypes || 1} 類訊號交叉評估；分數代表選題優先度，不是虛構的搜尋量。`,
      ...
      evidenceTypes,
      evidenceCount: newsMatches.length + literatureMatches.length + Math.min(suggestions.length, 5),
