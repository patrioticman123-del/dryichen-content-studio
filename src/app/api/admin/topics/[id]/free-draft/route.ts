...
import { FREE_DRAFT_MODEL_LABEL, freeDraftConfigured, generateFreeDraft } from '@/features/content-admin/free-draft';
...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  return NextResponse.json({ configured: freeDraftConfigured(), model: FREE_DRAFT_MODEL_LABEL, draft: await getFreeDraft(params.id) }, { headers: { 'Cache-Control': 'no-store' } });
}
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const generated = await generateFreeDraft(topic);
    const draft = await finishFreeDraft(topic.id, requestId, generated);
